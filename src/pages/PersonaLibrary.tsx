...
export default function PersonaLibrary({ onSubPageChange }: { onSubPageChange?: (isSubPage: boolean) => void }) {
  ...
  type Persona = {
    ...
    name: string;
    tags: string[];
    /** 批量打标等用户追加的标签，与 seed 中的 tags（原始人设标签）区分 */
    customTags: string[];
    score: number;
    conf: number;
    category: string;
  ...
  ] as Array<Omit<Persona, 'provenance' | 'customTags'>>).map((p, idx) => ({
    ...p,
    customTags: [],
  ...
  const PERSONA_CARD_PREVIEW_TAG_MAX = 5;

  const getPersonaCardPreviewTags = (p: Persona): string[] => {
    const orig = p.tags.map((t) => t.trim()).filter(Boolean);
    const custom = p.customTags.map((t) => t.trim()).filter(Boolean);
    const customOnly = custom.filter((t) => !orig.includes(t));
    const max = PERSONA_CARD_PREVIEW_TAG_MAX;
    if (orig.length + customOnly.length <= max) {
      return [...orig, ...customOnly];
    }
    if (customOnly.length === 0) {
      return orig.slice(0, max);
    }
    if (orig.length === 0) {
      return customOnly.slice(-max);
    }
    const customShow = Math.min(customOnly.length, max - 1);
    const origShow = max - customShow;
    return [...orig.slice(0, origShow), ...customOnly.slice(-customShow)];
  };

  const getPersonaDisplayTags = (p: Persona) => {
    const text = `${p.category} ${p.subCategory} ${p.tags.join(' ')} ${p.customTags.join(' ')} ${p.cdpTags.join(' ')} ${p.voc}`.toLowerCase();
    ...
    setAllPersonas((prev) =>
      prev.map((persona) => {
        ...
        const existing = new Set(
          [...persona.tags, ...persona.customTags].map((t) => t.trim()).filter(Boolean),
        );
        if (existing.has(tag)) return persona;
        return {
          ...persona,
          customTags: [...persona.customTags, tag],
        };
      }),
    );
            ...
            <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <button
                  ...
                  className="shrink-0 rounded border border-white/15 bg-surface-hover px-3 py-1 text-sm font-semibold text-gray-200 hover:border-white/25 hover:text-white transition-colors"
                >
                  清空筛选条件
                </button>
              </div>
              ...
              >
                {isBatchSelectMode ? '退出批量打标' : '进入批量打标'}
              </button>
            </div>
