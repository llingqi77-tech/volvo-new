...
export type InterviewQuestionType = 'open' | 'single' | 'multi';

export type InterviewOutlineQuestion = {
  id: string;
  order: number;
  content: string;
  aiHint?: string;
...
import { normalizeInterviewOutlineStage } from './interviewOutlineSerialize';
...
export function readResearchProjects(): ResearchProject[] {
  ...
  try {
    ...
    // #endregion
    if (!raw) {
      const migrated = readLegacyProjects().map((proj) => ({
        ...proj,
        interviewOutlineStage: normalizeInterviewOutlineStage(proj, proj.interviewOutlineStage),
      }));
    ...
    return parsed.map((p) => {
      ...
      const withStages = {
        ...base,
        insightSearchReportStage: base.insightSearchReportStage ?? {
          report: '',
          autoGenerated: false,
        },
      ...
      return {
        ...withStages,
        interviewOutlineStage: normalizeInterviewOutlineStage(withStages, withStages.interviewOutlineStage),
      };
    });
  } catch {
    return [];
